...
xyplot(Ozone ~ Wind | Month, #y軸~x軸 | 分組依據
       data = airquality, layout = c(5, 1)) # 1 rows, 5 column
#課堂操作
...
qplot(FieldGoalsAttempted, data = NBA1415, fill = Position)

#qplot 只有x軸> histogram// x,y軸都有> 散布圖
```
...
- For things that only make sense globally, use `theme()` 
  - Example: `theme(legend.position = "none")`  #圖示
...
#上下順序些微不同，圖一樣
ggplot(NBA1415, aes(x = FieldGoalsAttempted, y = TotalPoints)) +
    facet_grid(Position~.)+geom_point()+geom_smooth(method='lm')
```
...
#if (!require('gpclib')){
    #install.packages("gpclib",method="source");library(gpclib)}
if (!require('rgeos')){
